...
output:
  html_document:
    ...
    toc_float: true
vignette: >
  %\VignetteIndexEntry{Introduction to R and RStudio}
...
If you are viewing this lab on https://cuny-epibios.github.io/PUBH614/, you can 
enter, edit, and run the R code directly in your web browser. Try it! Also try 
changing the value assigned to `x`, and try some different calculator-like commands.
...
You only need to *install* packages once, but you need to *load* them each time you relaunch R. In this lab, the packages are already installed, but you can install new packages using `install.packages("packagename")`. We load packages with the `library` function. The following line load the tidyverse library which provides many data manipulation and plotting functions that you will use into your working environment, and the openintro package
which provides the `arbuthnot` dataset used in this lab.
...
<div>
  <label for="user-input">Enter your answer here:</label><br>
  <textarea id="user-input" name="user-input" rows="10" cols="50"></textarea>
</div>
...
```{webr-r}
# Enter your solution here, using the comment symbol "#" to start lines of plain text.
```
<div>
