...
In this lab, you’ll be analysing data from the Human Freedom Index
reports. Your aim will be to summarize a few of the relationships within
the data both graphically and numerically in order to find which
...
**tidyverse** suite of packages. You will also use the **statsr**
package to select a regression line that minimizes the sum of squared
residuals and the **broom** package to tidy regression output. The data
can be found in the **openintro** package, a companion package for
OpenIntro resources.
...
2.  The dataset spans a lot of years, but we are only interested in data
    from year 2016. Filter the data `hfi` data frame for year 2016,
    select the six variables, and assign the result to a data frame
    named `hfi_2016`.
...
3.  What type of plot would you use to display the relationship between
    ...
    Plot this relationship using the variable `pf_expression_control` as
    the predictor. Does the relationship look linear? If you knew a
    country’s `pf_expression_control`, or its score out of 10, with 0
    being the most, of political pressures and controls on media
    content, would you be comfortable using a linear model to predict
...
environment. You will also need to make sure the Plots tab in the lower
right-hand corner has enough space to make a plot.
...
4.  Looking at your plot from the previous exercise, describe the
...
will be shown in black and the residuals in blue.

If your plot is appearing below your code chunk and won’t let you select
points to make a line, take the following steps:
...
5.  Using `plot_ss`, choose a line that does a good job of minimizing
...
[`lm()`](https://rdrr.io/r/stats/lm.html) is a formula that takes the
...
**Note:** Piping **will not** work here, as the data frame is not the
first argument!

The output of [`lm()`](https://rdrr.io/r/stats/lm.html) is an object
that contains all of the information we need about the linear model that
was just fit. We can access this information using the `tidy()`
function.
...
$$\widehat{y} = 4.28 + 0.542 \times pf\_ expression\_ control$$
...
We can assess model fit using $R^{2}$, the proportion of variability in
the response variable that is explained by the explanatory variable. We
use the `glance()` function to access this information.

Please enable JavaScript to experience the dynamic code cell content on
this page.

For this model, 71.4% of the variability in `pf_score` is explained by
`pf_expression_control`.

6.  Fit a new model that uses `pf_expression_control` to predict
...
7.  If someone saw the least squares regression line and not the actual
    data, how would they predict a country’s personal freedom school for
    one with a 3 rating for `pf_expression_control`? Is this an
...
values and the residuals. We can use the `augment()` function to
calculate these.
...
it are the fitted values ($\widehat{y}$) and the residuals. Also note
...
- How does this relationship compare to the relationship between
  `pf_score` and `pf_expression_control`? Use the $R^{2}$ values from
  the two model summaries to compare. Does your independent variable
  seem to predict `pf_score` better? Why or why not?
