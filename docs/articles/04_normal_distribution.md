...
column of calories from fat. The argument `col` simply sets the color
for the line to be drawn. If we left it out, the line would be drawn in
black.
...
`dairy_queen` data set using the
[`nrow()`](https://rdrr.io/r/base/nrow.html) function. The last two
...
3.  Make a normal probability plot of `sim_norm`. Do all of the points
    ...
    for the real data? (Since `sim_norm` is not a dataframe, it can be
...
Note: you can use the double colon `::` to tell R explicitly from which
package to use a function. It is not necessary in this case to specify
that the function *qqnormsim* comes from the *openintro* package,
because the *openintro* package is already loaded. But if two packages
define functions of the same name, you can use `::` to tell R (and
anyone reading your code) which package the function you want to use
comes from. If you load two packages that define functions of the same
name, the second package loaded will “mask” the function from the first,
and without use of `::`, the function from the last package loaded will
take precedent. A very common example of this is the *select* function,
which is defined in *base* R and by the *dplyr* library, so when you
load [`library(dplyr)`](https://dplyr.tidyverse.org), its definition of
`select` replaces that defined in base R.
...
with the function [`pnorm()`](https://rdrr.io/r/stats/Normal.html).

Please enable JavaScript to experience the dynamic code cell content on
this page.

Note that the function [`pnorm()`](https://rdrr.io/r/stats/Normal.html)
gives the area under the normal curve below a given value, `q`, with a
given mean and standard deviation. Since we’re interested in the
probability that a Dairy Queen item has more than 600 calories from fat,
