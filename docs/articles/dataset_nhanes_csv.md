...
We take advantage of the ability of `haven`’s data-loading functions to
read data directly from a URL. Most other R data-loading functions can
do this too. The following command downloads and reads the dataset on
the first line, joins it with the dataset on the second line, and then
joins the result with the dataset on the third line.
